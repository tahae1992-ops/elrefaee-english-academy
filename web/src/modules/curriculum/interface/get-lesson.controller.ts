import type { GetLessonUseCase } from "@/modules/curriculum/application/use-cases/get-lesson.use-case";
import { LessonNotFoundError } from "@/modules/curriculum/application/use-cases/get-lesson.use-case";

// API Spec §6.3: GET /lessons/{id}.
export async function handleGetLesson(useCase: GetLessonUseCase, lessonId: string): Promise<{ status: number; body: unknown }> {
  try {
    const result = await useCase.execute(lessonId);
    return { status: 200, body: result };
  } catch (error) {
    if (error instanceof LessonNotFoundError) return { status: 404, body: { error: "NOT_FOUND", message: error.message } };

    console.error("GET /api/v1/lessons/[id] failed:", error);
    const Sentry = await import("@sentry/nextjs");
    Sentry.captureException(error);
    return { status: 500, body: { error: "INTERNAL", message: "Could not load lesson." } };
  }
}
