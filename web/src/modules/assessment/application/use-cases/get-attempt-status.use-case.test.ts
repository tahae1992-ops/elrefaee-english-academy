...
describe("GetAttemptStatusUseCase", () => {
  it("returns the placement result for a placement-kind attempt", async () => {
    ...
    const status = await new GetAttemptStatusUseCase(attempts, itemBank, results, checkpointResults).execute("attempt-1", "user-1");
  ...
  it("throws AttemptNotFoundError when the attempt doesn't exist", async () => {
    ...
    await expect(new GetAttemptStatusUseCase(attempts, itemBank, results, checkpointResults).execute("attempt-1", "user-1")).rejects.toThrow(
