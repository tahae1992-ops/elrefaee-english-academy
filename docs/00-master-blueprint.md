...
**Revised 2026-08-03, mid-implementation (Sprint 1 complete, before Sprint 2):** the original decision below ("i18n-ready, English-only at launch, build later") stands in substance — but "later" is now **now**, at the user's explicit request, rather than deferred to a future phase. The scope boundary (what gets translated vs. what doesn't) is unchanged and still load-bearing; only the *timing* of building the infrastructure moved earlier. This section is the authoritative, current version; treat any earlier reference to localization as "deferred" elsewhere in this document series as superseded by this revision.

**Requirement:** the *interface* (navigation, buttons, dashboards, marketing site, instructional chrome, notifications, emails, and certificate template text) must support unlimited languages, with automatic locale detection and manual override, and both LTR and RTL layouts — while the *curriculum content itself* (lesson bodies, reading passages, quiz questions, grammar explanations — the actual English being taught) stays in English regardless of interface language, since that's the product's subject matter, not its chrome. This scope line is unchanged from the original decision and was reconfirmed explicitly rather than assumed when the request to build this now arrived.

**Architectural separation (unchanged, now load-bearing sooner than planned):**
- **Static UI strings** — navigation, buttons, form labels, error messages, marketing copy — live as `next-intl` message catalogs (one JSON file per locale, version-controlled with code), never a database round-trip for something that changes only on deploy.
- **Dynamic, admin-editable translated content** — notification templates, transactional email templates, and certificate template text (the disclaimer, level-name labels, headings — not the learner's actual per-skill scores, which are data, not text to translate) — live as **database-backed translation tables**, because these need to be editable by an Academy Admin without a code deploy, unlike static UI chrome. See DDD addendum (Section 5's cross-reference) for the concrete schema.
- **Curriculum content** — lessons, vocabulary, exercises, quiz/exam items — is **still not translated**; this line has not moved. The only place a learner's L1 might ever enter the content layer remains the optional, still-out-of-scope glossing feature from the original decision.
- **AI Tutor / Conversation Partner** — responds in English always, regardless of interface language, consistent with EDD §18's immersion-modeling methodology. A learner using a Spanish interface still receives English-language tutoring by design; this was confirmed explicitly, not assumed, when this revision was scoped.
- **Locale identity is an open, growing set, not a closed one** — represented as a validated BCP-47 string (e.g. `en`, `es`, `ar`), never a Postgres enum. This is a deliberate, stated contrast with genuinely closed sets elsewhere in this system (like `cefr_level`): adding a new supported language must be a data change, never a migration, to actually satisfy "unlimited languages."
- **RTL** — the design system (doc 07) is built with logical CSS properties (`margin-inline-start`, not `margin-left`) throughout, so RTL layout is structurally supported from day one. No specific RTL locale is targeted yet, so full visual polish/verification under a real RTL locale is deferred to when one is actually added — the capability is real now, the verification isn't, and that distinction is deliberate, not a gap being hidden.

**Why this was worth stopping to clarify rather than just building:** the literal request ("translation tables for courses, lessons, quizzes, certificates") could have been read as reversing the original "curriculum content stays English" decision — a fundamentally different, much larger content-production commitment. Confirming the scope stayed at the chrome/dynamic-content layer (not pedagogical content) before touching any schema or already-shipped Sprint 1 code was the entire point of stopping to ask.
