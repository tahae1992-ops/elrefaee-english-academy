...
**Internal Components:** Next.js App Router route groups mirroring bounded contexts (`/app/[locale]/(student)`, `/app/[locale]/(instructor)`, `/app/[locale]/(studio)` for Curriculum Studio, `/app/[locale]/(admin)` — the `[locale]` segment **added 2026-08-03**, per Blueprint §12's revised, brought-forward i18n decision; `next-intl`'s routing middleware/`proxy.ts` resolves the segment per DDD §3.12's locale-resolution order before any route group renders, and API routes under `/app/api/**` deliberately sit *outside* this segment — the API contract is locale-agnostic by design, per API Spec §1's conventions, with locale expressed as an `Accept-Language` header or an explicit query param where a response body genuinely needs localized text, e.g. a rendered notification); a shared design-system package (Tailwind/shadcn tokens, Blueprint §17, built on logical CSS properties per doc 07 §3's RTL addendum) consumed by every route group so dark mode/high-contrast/dyslexia-font/RTL (Blueprint §11, §12) are implemented once; TanStack Query for client-side server-state caching/revalidation; React Server Components by default, Client Components only where interactivity requires it (minimizes shipped JS, directly serves the p95 performance NFR, SRS §3).
